...
@pytest.mark.parametrize("arg_type", [list, list[int], set[int]])
def test_list_arg(arg_type):
    """
    Tests that a collection argument does not pull from the path
    """

    @api_view
    def test_view(request, data: arg_type) -> int:
        return list(data)[0]

    factory = RequestFactory()

    response = test_view(factory.get("/test/", {"data": "2"}), data=1)
    assert json.loads(response.content) == 2
